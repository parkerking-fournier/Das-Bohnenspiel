...
public abstract class Player {
     ...
     * player's turn.
     *
     * @param board_state The current board state, from the perspective
     *    of the player that owns the board. This is a copy of the
     *    board state maintained by the server, so it may be modified without
    ...
    public void setColor( int c ) { player_id = c; }

    final public void setName( String s ) {
        name = s.length()<=0 ? "<none>": s;
    }

    final public int getColor() { return player_id; }
